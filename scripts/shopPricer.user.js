...
(() => {
    ...
    */
    ...
    const someSelector = doc.querySelector(".middleit.bigger .petpadding")
    ...
    else if (idPage && !buyPage && !someSelector) {
        findNewStore()
    }
    ...
    if (!idPage) {
        ...
        if (!captcha) { button.click() } // No captcha buy the item
        else if (!button) { shopkeeper.click() } // "Sorry just sold the last..."
        ...
        const IN_SHOP_ID = shopkeeper.href.split("id=")[1] // Shop ID from the URL
